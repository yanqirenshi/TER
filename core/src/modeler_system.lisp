...
(defun find-r-modeler2system (graph modeler)
  (shinra:find-r graph 'edge :from modeler
                             :edge-type :select-at
                             :vertex-class 'system))

(defun get-r-modeler2system (graph modeler &key system)
  (let ((r-list (find-r-modeler2system graph modeler)))
...
(defun get-edge-modeler2system (graph modeler &key system)
  (getf (get-r-modeler2system graph modeler :system system)
        :edge))

(defgeneric tx-make-edge-modeler2system (graph modeler system)
...
(defgeneric tx-change-select-system (graph modeler system)
  (:method (graph modeler system)
    (unless (get-r-modeler2system graph modeler :system system)
      (let ((edge (get-edge-modeler2system graph modeler)))
        (if edge
            (tx-change-system-at-modeler2system graph edge system)
            (tx-make-edge-modeler2system graph modeler system))))))
