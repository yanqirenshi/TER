...
(defun parse-attribute-line-constant (line)
...
(defun parse-attribute-line-foreign-key-item (line)
  (let ((pos (position #\: line)))
    (list (alexandria:make-keyword (string-upcase (trim (subseq line 0 pos))))
          (rm-dbl-quote (trim (subseq line (+ 1 pos)))))))

(defun parse-attribute-line-foreign-key-items (lines)
  (alexandria:when-let ((line (car lines)))
    (nconc (parse-attribute-line-foreign-key-item line)
           (parse-attribute-line-foreign-key-items (cdr lines)))))

(defun parse-attribute-line-foreign-key (line)
  (multiple-value-bind (ret arr)
      (cl-ppcre:scan-to-strings "foreign_key:\\s*{([^}]+)}.*" line)
    (when ret
      (list :foreign-key
            (parse-attribute-line-foreign-key-items (split-sequence:split-sequence #\, (aref arr 0)))))))

(defun parse-attribute-line-limit (line)
  (multiple-value-bind (ret arr)
...
(defun parse-attribute-line (line)
  (multiple-value-bind (constant options)
      (split-attribute-line line)
    (nconc (nconc (parse-attribute-line-constant constant)
                  (parse-attribute-line-foreign-key options))
           (parse-attribute-line-limit line))))
